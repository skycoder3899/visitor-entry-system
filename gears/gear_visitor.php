<?php include "../cores/inc/config.php";
 $output = '';  
 $sql = "SELECT * FROM `visitor_tbl` v INNER JOIN employee_dept_tbl d ON v.host_department_id=d.department_id INNER JOIN employee_tbl e on v.host_id=e.e_id AND DATE(v.date)=CURDATE()";  
 $result = mysqli_query($link, $sql); 
 $sql = "SELECT * FROM `visitor_tbl` v INNER JOIN employee_dept_tbl d ON v.host_department_id=d.department_id INNER JOIN employee_tbl e on v.host_id=e.e_id AND DATE(v.date)!=CURDATE()";  
 $result1 = mysqli_query($link, $sql);  
 if(mysqli_num_rows($result) > 0 || mysqli_num_rows($result1) > 0)  
 {
 $output .= '<script>$("#Table2").DataTable({dom: "Bfrtip",buttons:["excel",  {
            extend: "pdfHtml5",
